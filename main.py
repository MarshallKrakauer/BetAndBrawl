...
if __name__ == '__main__':
    ...
    results = []
    combos = list(product(FIGHT_LENGTHS, PUNCH_KO_THRESHOLDS, TKO_THRESHOLDS, METER_MAXES, STARTING_METERS))
    print(f"Running {len(combos)} parameter combinations...")

    for bout_length, punch_ko_thresh, tko_thresh, meter_max, (red_meter, blue_meter) in combos:
        df = simulate_fights(
            num_fights=10_000,
            red_corner_starting_meter=red_meter,
            blue_corner_starting_meter=blue_meter,
            tko_threshold=tko_thresh,
